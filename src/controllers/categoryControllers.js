...
exports.createCategory = async(req, res) => {
    ...
    try {
        let categoryName = await Category.findOne({ category });
         console.log("no ha pasasdo en el try")
        if(categoryName){
...
exports.queryCategory = async(req, res) => {
    Category.find({}, (err, categorys) => {
        if (err) {
            ...
            return res.status(500).json({ message: `Erro this petition ${err}` })
        }
        if (!categorys) {
            return res.status(404).json({ message: `The categorys not exist ` })
...
exports.queryCategoryId = async(req, res) => {
    ...
    Category.findById(categoryId, (err, category) => {
        ...
        if (!category) {
            return res.status(404).json({ message: `The category not exist ` })
...
exports.updateCategoryId = async(req, res) => {
    ...
    Category.findByIdAndUpdate(categoryId, update, (err, category) => {
        if (err) {
            ...
            return res.status(500).json({ message: `Erro on update category ${err}` })
        }
...
exports.deleteCategoryId=async(req,res)=>{
    let categoryId=req.params.IdCategory
     Category.findById(categoryId,(err,category)=>{
         if(err) res.status(500).json({message:`Error on delete Category${err}`})

         category.remove(err=>{
             if(err) res.status(500).send({message: 'Erro this delete category'})
             res.status(200).json({message:'This product has delete'})
         })
     })
}
